...
public class FecharPortao : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            grade.transform.rotation = Quaternion.RotateTowards(grade.transform.rotation, rotacaoAlvo, velocidade * Time.deltaTime);
        }
    }

    private void OnTriggerExit2D(Collider2D collision)
    {
        if (collision.gameObject.layer == 10 && fechouPortao == false)
        {
            fechouPortao = true;
